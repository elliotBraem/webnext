...
use near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize},
               ...
               serde_json, AccountId, Balance, CryptoHash, Gas};
...
/// Factory manager that allows to store/load contracts by hash directly in the
/// storage. Uses directly underlying host functions to not load any of the data
/// into WASM memory.
...
impl FactoryManager {
	...
	pub fn store_contract(&self) {
		...
		let blob_hash_str =
			serde_json::to_string(&Base58CryptoHash::from(blob_hash)).unwrap().into_bytes();
	...
	pub fn update_contract(&self, account_id: AccountId, code_hash: Base58CryptoHash,
	                       method_name: &str) {
		let code_hash: CryptoHash = code_hash.into();
		// Check that such contract exists.
		assert!(env::storage_has_key(&code_hash), "Contract doesn't exist");
	...
	pub fn create_contract(&self, code_hash: Base58CryptoHash, account_id: AccountId,
	                       new_method: &str, args: &[u8], callback_method: &str,
	                       callback_args: &[u8]) {
		let code_hash: CryptoHash = code_hash.into();
		...
		assert!(attached_deposit >= storage_cost, "ERR_NOT_ENOUGH_DEPOSIT:{}", storage_cost);
		...
		env::promise_batch_action_function_call(
		                                        promise_id, new_method, args, NO_DEPOSIT,
		                                        CREATE_CALL_GAS,
		);
		// attach callback to the factory.
		let _ = env::promise_then(
		                          promise_id, factory_account_id, callback_method, callback_args,
		                          NO_DEPOSIT, ON_CREATE_CALL_GAS,
		);
		env::promise_return(promise_id);
	}
}
