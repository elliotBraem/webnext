...
impl SputnikDAOFactory {
	#[init]
	pub fn new() -> Self {
		let this = Self { factory_manager: FactoryManager {},
	...
	fn internal_store_initial_contract(&self) {
		...
		self.store_contract_metadata(
		                             ...
		                             DaoContractMetadata { version:       DAO_CONTRACT_INITIAL_VERSION,
		                                                   commit_id:     String::from(
			DAO_CONTRACT_NO_DATA,
		),
		                                                   changelog_url: None, },
		                             true,
		);
	...
	pub fn create(&mut self, name: AccountId, args: Base64VecU8) {
		let account_id: AccountId = format!("{}.{}", name, env::current_account_id()).parse().unwrap();
	...
	#[private]
	pub fn on_create(&mut self, account_id: AccountId, attached_deposit: U128,
	...
	/// Tries to update given account created by this factory to the specified
	/// code.
	pub fn update(&self, account_id: AccountId, code_hash: Base58CryptoHash) {
		...
		assert!(self.daos.contains(&account_id), "Must be contract created by factory");
		self.factory_manager.update_contract(account_id, code_hash, "update");
	}

	/// Allows a DAO to store the official factory version as a blob, funded by
	/// the DAO wanting to upgrade Required to successfully upgrade a DAO via
	/// proposals (proposal to store blob, proposal to upgrade from local blob)
	/// Only intended for sputnik v2 DAO's created by sputnik factory
	/// Payment is needed to cover storage costs for code blob size, paid by the
	/// DAO and returned upon blob removal
	#[payable]
	...
	/// Allows a DAO to remove the blob stored in its DAO storage, and reclaim the
	/// storage cost
	pub fn remove_contract_self(&mut self, code_hash: Base58CryptoHash) {
		...
		let callback_args =
			&json!({ "account_id": &account_id, "code_hash": &code_hash }).to_string().into_bytes();

		// Create a promise toward given account.
		let promise_id = env::promise_batch_create(&account_id);
		env::promise_batch_action_function_call(
		                                        promise_id, method_name, method_args, NO_DEPOSIT,
		                                        GAS_STORE_CONTRACT_LEFTOVER,
		);
		// attach callback to the factory.
		let _ = env::promise_then(
		                          promise_id, factory_id, callback_method, callback_args, NO_DEPOSIT,
		                          ON_REMOVE_CONTRACT_GAS,
		);
		env::promise_return(promise_id);
	}

	/// Upon blob remove, compute the balance (if any) that got paid to the
	/// factory, since it was the "owner" of the blob stored on the DAO.
	/// Send this balance back to the DAO, since it was the original funder
	#[private]
	pub fn on_remove_contract_self(&mut self, account_id: AccountId, code_hash: Base58CryptoHash)
	                               -> bool {
		if near_sdk::is_promise_success() {
	...
	pub fn get_daos(&self, from_index: u64, limit: u64) -> Vec<AccountId> {
		...
		(from_index .. std::cmp::min(from_index + limit, elements.len())).filter_map(|index| {
			                                                                 elements.get(index)
		                                                                 })
		                                                                 .collect()
	}
	...
	pub fn get_default_version(&self) -> Version {
		...
		let default_metadata =
			deserialized_metadata.get(&self.get_default_code_hash()).expect("INTERNAL_FAIL");
	...
	pub fn store_contract_metadata(&self, code_hash: Base58CryptoHash,
	                               metadata: DaoContractMetadata, set_default: bool) {
		self.assert_owner();
		let hash: CryptoHash = code_hash.into();
		assert!(
		        env::storage_has_key(&hash),
	...
	fn assert_owner(&self) {
		assert_eq!(self.get_owner(), env::predecessor_account_id(), "Must be owner");
	}
}
...
pub extern "C" fn store() {
	...
	assert!(storage_cost <= env::attached_deposit(),
	        "Must at least deposit {} to store",
	        storage_cost);
}
...
mod tests {
	...
	fn test_basics() {
		...
		assert_eq!(factory.get_dao_list(), vec![format!("test.{}", accounts(0)).parse().unwrap()]);
		assert_eq!(factory.get_daos(0, 100), vec![format!("test.{}", accounts(0)).parse().unwrap()]);
	}
	...
	fn test_factory_can_get_current_owner() {
		let mut context = VMContextBuilder::new();
		testing_env!(context.current_account_id(alice())
	...
	fn test_owner_can_be_a_dao_account() {
		let mut context = VMContextBuilder::new();
		testing_env!(context.current_account_id(bob())
		...
		assert_eq!(factory.get_owner(), AccountId::new_unchecked("bob.sputnik-dao.near".to_string()))
	}
