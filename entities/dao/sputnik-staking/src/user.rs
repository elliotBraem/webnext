use {crate::*,
     near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize},
                ...
                AccountId, Balance, Duration, StorageUsage}};
...
/// Once delegated - the tokens are used in the votes. It records for each
/// delegate when was the last vote. When undelegating - the new delegations or
/// withdrawal are only available after cooldown period from last vote of the
/// delegate.
...
impl User {
	pub fn new(near_amount: Balance) -> Self {
		Self { storage_used:          Self::min_storage(),
	...
	/// They are deposited on internal_register and removed on
	/// internal_unregister.
	...
	pub(crate) fn delegated_amount(&self) -> Balance {
		self.delegated_amounts.iter().fold(0, |total, (_, amount)| total + amount.0)
	}
	...
	pub fn delegate(&mut self, delegate_id: AccountId, amount: Balance) {
		assert!(self.delegated_amount() + amount <= self.vote_amount.0, "ERR_NOT_ENOUGH_AMOUNT");
		assert!(env::block_timestamp() >= self.next_action_timestamp.0, "ERR_NOT_ENOUGH_TIME_PASSED");
		self.storage_used += delegate_id.as_bytes().len() as StorageUsage + U128_LEN;
		self.delegated_amounts.push((delegate_id, U128(amount)));
		self.assert_storage();
	}

	/// Remove given amount from delegates. Updates timestamp when next action can
	/// be called. Fails if delegate not found or not enough amount delegated.
	pub fn undelegate(&mut self, delegate_id: &AccountId, amount: Balance,
	                  undelegation_period: Duration) {
		let f = self.delegated_amounts
		            .iter()
		            .enumerate()
...
impl Contract {
	...
	pub fn internal_get_user_opt(&self, account_id: &AccountId) -> Option<User> {
		self.users.get(account_id).map(|versioned_user| match versioned_user {
	...
	pub fn internal_delegate(&mut self, sender_id: AccountId, delegate_id: AccountId,
	                         amount: Balance) {
		let mut sender = self.internal_get_user(&sender_id);
	...
	pub fn internal_undelegate(&mut self, sender_id: AccountId, delegate_id: AccountId,
	                           amount: Balance) {
		let mut sender = self.internal_get_user(&sender_id);
