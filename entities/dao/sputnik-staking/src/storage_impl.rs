...
impl StorageManagement for Contract {
	...
	fn storage_balance_bounds(&self) -> StorageBalanceBounds {
		StorageBalanceBounds { min: U128(User::min_storage() as Balance * env::storage_byte_cost()),
		                       max: None, }
	}

	fn storage_balance_of(&self, account_id: AccountId) -> Option<StorageBalance> {
		self.internal_get_user_opt(&account_id).map(|user| {
			                                       StorageBalance { total:     user.near_amount,
			                                                        available:
				                                                        U128(user.storage_available()), }
		                                       })
	}
}
