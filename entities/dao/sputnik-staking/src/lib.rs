...
use {near_contract_standards::fungible_token::{core_impl::ext_fungible_token,
     ...
     near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize},
                ...
                env, ext_contract,
                json_types::{U128, U64},
                near_bindgen, AccountId, Balance, BorshStorageKey, Duration, Gas, PanicOnDefault,
...
impl Contract {
	...
	pub fn delegate(&mut self, account_id: AccountId, amount: U128) -> Promise {
		...
		ext_sputnik::delegate(account_id.into(), amount, self.owner_id.clone(), 0, GAS_FOR_DELEGATE)
	}
	...
	pub fn undelegate(&mut self, account_id: AccountId, amount: U128) -> Promise {
		...
		ext_sputnik::undelegate(account_id.into(), amount, self.owner_id.clone(), 0, GAS_FOR_UNDELEGATE)
	}
	...
	pub fn exchange_callback_post_withdraw(&mut self, sender_id: AccountId, amount: U128) {
		assert_eq!(env::promise_results_count(), 1, "ERR_CALLBACK_POST_WITHDRAW_INVALID",);
...
impl FungibleTokenReceiver for Contract {
	fn ft_on_transfer(&mut self, sender_id: AccountId, amount: U128, msg: String)
	                  -> PromiseOrValue<U128> {
		assert_eq!(self.vote_token_id, env::predecessor_account_id(), "ERR_INVALID_TOKEN");
...
mod tests {
	use {super::*,
	     near_contract_standards::storage_management::StorageManagement,
	...
	fn test_basics() {
		...
		testing_env!(context.predecessor_account_id(contract_owner.clone()).build());
		...
		contract.ft_on_transfer(delegate_from_user.clone(), U128(to_yocto("100")), "".to_string());
		assert_eq!(contract.ft_total_supply().0, to_yocto("100"));
		assert_eq!(contract.ft_balance_of(delegate_from_user.clone()).0, to_yocto("100"));

		testing_env!(context.predecessor_account_id(delegate_from_user.clone()).build());
		contract.withdraw(U128(to_yocto("50")));
		assert_eq!(contract.ft_total_supply().0, to_yocto("50"));
		assert_eq!(contract.ft_balance_of(delegate_from_user.clone()).0, to_yocto("50"));
