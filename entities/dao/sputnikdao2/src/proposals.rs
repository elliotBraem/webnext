use {crate::{policy::UserInfo,
             types::{convert_old_to_new_token, Action, Config, OldAccountId,
                     GAS_FOR_FT_TRANSFER, OLD_BASE_TOKEN, ONE_YOCTO_NEAR},
             upgrade::{upgrade_remote, upgrade_using_factory},
             *},
     near_contract_standards::fungible_token::core_impl::ext_fungible_token,
     near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize},
                json_types::{Base64VecU8, U128, U64},
                log, AccountId, Balance, Gas, PromiseOrValue},
...
pub enum ProposalStatus {
	...
	Rejected,
	/// If quorum voted to remove (e.g. spam), this proposal is rejected and bond
	/// is not returned. Interfaces shouldn't show removed proposals.
	Removed,
	/// Expired after period of time.
	Expired,
	/// If proposal was moved to Hub or somewhere else.
	Moved,
	/// If proposal has failed when finalizing. Allowed to re-finalize again to
	/// either expire or approved.
	Failed,
}
...
pub enum ProposalKind {
	...
	/// Add member to given role in the policy. This is short cut to updating the
	/// whole policy.
	AddMemberToRole { member_id: AccountId, role: String },
	/// Remove member to given role in the policy. This is short cut to updating
	/// the whole policy.
	RemoveMemberFromRole { member_id: AccountId, role: String },
	/// Calls `receiver_id` with list of method names in a single promise.
	/// Allows this contract to execute any arbitrary set of actions in other
	/// contracts.
	FunctionCall { receiver_id: AccountId, actions: Vec<ActionCall> },
	/// Upgrade this contract with given hash from blob store.
	UpgradeSelf { hash: Base58CryptoHash },
	/// Upgrade another contract, by calling method with the code from given hash
	/// from blob store.
	UpgradeRemote { receiver_id: AccountId, method_name: String, hash: Base58CryptoHash },
	/// Transfers given amount of `token_id` from this DAO to `receiver_id`.
	/// If `msg` is not None, calls `ft_transfer_call` with given `msg`. Fails if
	/// this base token. For `ft_transfer` and `ft_transfer_call` `memo` is the
	...
	/// Sets staking contract. Can only be proposed if staking contract is not set
	/// yet.
	...
	/// Add new role to the policy. If the role already exists, update it. This is
	/// short cut to updating the whole policy.
	ChangePolicyAddOrUpdateRole { role: RolePermission },
	/// Remove role from the policy. This is short cut to updating the whole
	/// policy.
	ChangePolicyRemoveRole { role: String },
	/// Update the default vote policy from the policy. This is short cut to
	/// updating the whole policy.
	ChangePolicyUpdateDefaultVotePolicy { vote_policy: VotePolicy },
	/// Update the parameters from the policy. This is short cut to updating the
	/// whole policy.
...
impl Proposal {
	/// Adds vote of the given user with given `amount` of weight. If user already
	/// voted, fails.
	pub fn update_votes(&mut self, account_id: &AccountId, roles: &[String], vote: Vote,
	                    policy: &Policy, user_weight: Balance) {
		for role in roles {
		...
		assert!(self.votes.insert(account_id.clone(), vote).is_none(), "ERR_ALREADY_VOTED");
	}
}
...
impl From<ProposalInput> for Proposal {
	fn from(input: ProposalInput) -> Self {
		Self { proposer:        env::predecessor_account_id(),
...
impl Contract {
	...
	pub(crate) fn internal_payout(&mut self, token_id: &Option<AccountId>,
	                              receiver_id: &AccountId, amount: Balance, memo: String,
	                              msg: Option<String>)
	                              -> PromiseOrValue<()> {
		if token_id.is_none() {
	...
	fn internal_execute_proposal(&mut self, policy: &Policy, proposal: &Proposal, proposal_id: u64)
	                             -> PromiseOrValue<()> {
		let result = match &proposal.kind {
			...
			| ProposalKind::UpgradeRemote { receiver_id, method_name, hash } => {
				upgrade_remote(&receiver_id, method_name, &CryptoHash::from(hash.clone()));
				PromiseOrValue::Value(())
			},
			| ProposalKind::Transfer { token_id, receiver_id, amount, msg } => {
		...
		match result {
			| PromiseOrValue::Promise(promise) => {
				promise.then(ext_self::on_proposal_callback(
					proposal_id,
	...
	pub(crate) fn internal_callback_proposal_success(&mut self, proposal: &mut Proposal)
	                                                 -> PromiseOrValue<()> {
	...
	pub(crate) fn internal_callback_proposal_fail(&mut self, proposal: &mut Proposal)
	                                              -> PromiseOrValue<()> {
	...
	fn internal_reject_proposal(&mut self, policy: &Policy, proposal: &Proposal,
	                            return_bonds: bool)
	                            -> PromiseOrValue<()> {
		if return_bonds {
	...
	pub(crate) fn internal_user_info(&self) -> UserInfo {
		...
		UserInfo { amount: self.get_user_weight(&account_id), account_id }
	}
}
...
impl Contract {
	...
	pub fn add_proposal(&mut self, proposal: ProposalInput) -> u64 {
		...
		assert_eq!(env::attached_deposit(), policy.proposal_bond.0, "ERR_MIN_BOND");
		...
		match &proposal.kind {
			...
			| ProposalKind::Transfer { token_id, msg, .. } => {
				assert!(!(token_id == OLD_BASE_TOKEN) || msg.is_none(), "ERR_BASE_TOKEN_NO_MSG");
			},
			| ProposalKind::SetStakingContract { .. } => {
				assert!(self.staking_id.is_none(), "ERR_STAKING_CONTRACT_CANT_CHANGE")
			},
		...
		self.proposals.insert(&id, &VersionedProposal::Default(proposal.into()));
	...
	/// Memo is logged but not stored in the state. Can be used to leave notes or
	/// explain the action.
	pub fn act_proposal(&mut self, id: u64, action: Action, memo: Option<String>) {
		...
		let update = match action {
			...
			// In case of failed, we just recompute the status and if it still approved, we re-execute the
			// proposal. In case of expired, we reject the proposal and return the bond.
			// Corner cases:
			//  - if proposal expired during the failed state - it will be marked as expired.
			//  - if the number of votes in the group has changed (new members has been added) - the
			//    proposal can loose it's approved state. In this case new proposal needs to be made, this
			//    one can only expire.
			| Action::Finalize => {
				proposal.status =
					policy.proposal_status(
					                       &proposal,
		...
		if update {
			self.proposals.insert(&id, &VersionedProposal::Default(proposal));
		}
	...
	/// If the proposal execution failed (funds didn't transfer or function call
	/// failure), move proposal to "Failed" state.
	#[private]
