...
pub enum RoleKind {
	...
	Everyone,
	/// Member greater or equal than given balance. Can use `1` as non-zero
	/// balance.
	Member(U128),
	/// Set of accounts.
...
impl RoleKind {
	...
	/// Returns the number of people in the this role or None if not supported
	/// role kind.
	...
	pub fn to_weight(&self, total_weight: Balance) -> Balance {
		match self {
			...
			| WeightOrRatio::Ratio(num, denom) => {
				min((*num as u128 * total_weight) / *denom as u128 + 1, total_weight)
			},
		}
	}
}
...
pub enum WeightKind {
	...
	TokenWeight,
	/// Weight of the group role. Roles that don't have scoped group are not
	/// supported.
	RoleWeight,
}
...
pub struct VotePolicy {
	...
	///     This allows to avoid situation where the number of staked tokens from
	/// total supply is too small. If RoleWeight - this is minimum number of
	/// votes.     This allows to avoid situation where the role is got too small
	/// but policy kept at 1/2, for example.
	pub quorum:      U128,
...
impl Default for VotePolicy {
	fn default() -> Self {
		VotePolicy { weight_kind: WeightKind::RoleWeight,
		             quorum:      U128(0),
...
pub struct Policy {
	...
	/// Default vote policy. Used when given proposal kind doesn't have special
	/// policy.
...
impl VersionedPolicy {
	...
	pub fn upgrade(self) -> Self {
		match self {
			| VersionedPolicy::Default(accounts) => VersionedPolicy::Current(default_policy(accounts)),
...
impl Policy {
	pub fn add_or_update_role(&mut self, role: &RolePermission) {
		for i in 0 .. self.roles.len() {
			if &self.roles[i].name == &role.name {
				env::log_str(&format!("Updating existing role in the policy:{}", &role.name));
	...
	/// Returns set of roles that this user is member of permissions for given
	/// user across all the roles it's member of.
	...
	pub fn can_execute_action(&self, user: UserInfo, proposal_kind: &ProposalKind, action: &Action)
	                          -> (Vec<String>, bool) {
	...
	pub fn proposal_status(&self, proposal: &Proposal, roles: Vec<String>, total_supply: Balance)
	                       -> ProposalStatus {
		assert!(
		        matches!(proposal.status, ProposalStatus::InProgress | ProposalStatus::Failed),
		...
		for role in roles {
			...
			let vote_policy = role_info.vote_policy
			...
			let threshold =
				std::cmp::max(vote_policy.quorum.0, vote_policy.threshold.to_weight(total_weight));
			// Check if there is anything voted above the threshold specified by policy for
			// given role.
...
#[cfg(test)]
mod tests {
	use {super::*, near_sdk::test_utils::accounts};
	...
	fn test_add_role() {
		...
		let new_role = RolePermission { name:        name.clone(),
		                                kind:        kind.clone(),
	...
	fn test_update_role() {
		...
		let kind: RoleKind = RoleKind::Group(vec![accounts(2), accounts(3)].into_iter().collect());
		let permissions: HashSet<String> = vec!["*:*".to_string()].into_iter().collect();
		let updated_role = RolePermission { name:        name.clone(),
		                                    kind:        kind.clone(),
		                                    permissions: permissions.clone(),
		                                    vote_policy: vote_policy.clone(), };
		assert_eq!(2, policy.roles.len());
	...
	fn test_update_default_vote_policy() {
		...
		assert_eq!(WeightKind::RoleWeight, policy.default_vote_policy.weight_kind);
		assert_eq!(U128(0), policy.default_vote_policy.quorum);
		assert_eq!(WeightOrRatio::Ratio(1, 2), policy.default_vote_policy.threshold);

		let new_default_vote_policy = VotePolicy { weight_kind: WeightKind::TokenWeight,
		                                           quorum:      U128(100),
		                                           threshold:   WeightOrRatio::Ratio(1, 4), };
		policy.update_default_vote_policy(&new_default_vote_policy);
		assert_eq!(new_default_vote_policy.weight_kind, policy.default_vote_policy.weight_kind);
		assert_eq!(new_default_vote_policy.quorum, policy.default_vote_policy.quorum);
		assert_eq!(new_default_vote_policy.threshold, policy.default_vote_policy.threshold);
	}
	...
	fn test_update_parameters() {
		...
		assert_eq!(U64::from(1_000_000_000 * 60 * 60 * 24 * 7), policy.proposal_period);
		assert_eq!(U128(10u128.pow(24)), policy.bounty_bond);
		assert_eq!(U64::from(1_000_000_000 * 60 * 60 * 24), policy.bounty_forgiveness_period);

		let new_parameters = PolicyParameters { proposal_bond:             Some(U128(10u128.pow(26))),
		                                        proposal_period:           None,
		                                        bounty_bond:               None,
		                                        bounty_forgiveness_period: Some(U64::from(
			1_000_000_000 * 60 * 60 * 24 * 5,
		)), };
		policy.update_parameters(&new_parameters);
		assert_eq!(U128(10u128.pow(26)), policy.proposal_bond);
		assert_eq!(U64::from(1_000_000_000 * 60 * 60 * 24 * 7), policy.proposal_period);
		assert_eq!(U128(10u128.pow(24)), policy.bounty_bond);
		assert_eq!(U64::from(1_000_000_000 * 60 * 60 * 24 * 5), policy.bounty_forgiveness_period);
	}
}
