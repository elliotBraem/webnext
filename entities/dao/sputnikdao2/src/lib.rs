use {crate::{proposals::VersionedProposal,
             upgrade::{internal_get_factory_info, internal_set_factory_info, FactoryInfo}},
...
pub use crate::{bounties::{Bounty, BountyClaim, VersionedBounty},
                policy::{default_policy, Policy, RoleKind, RolePermission, VersionedPolicy,
                         VotePolicy},
                proposals::{Proposal, ProposalInput, ProposalKind, ProposalStatus},
                types::{Action, Config, OldAccountId, OLD_BASE_TOKEN},
                views::{BountyOutput, ProposalOutput}};
...
pub struct Contract {
	...
	/// Bounty claimers map per user. Allows quickly to query for each users their
	/// claims.
...
impl Contract {
	...
	pub fn new(config: Config, policy: VersionedPolicy) -> Self {
		let this =
			Self { config:                  LazyOption::new(StorageKeys::Config, Some(&config)),
			       ...
			       locked_amount:           0, };
		internal_set_factory_info(&FactoryInfo { factory_id:  env::predecessor_account_id(),
		                                         auto_update: true, });
		this
	}

	/// Should only be called by this contract on migration.
	/// This is NOOP implementation. KEEP IT if you haven't changed contract
	/// state. If you have changed state, you need to implement migration from old
	/// state (keep the old struct with different name to deserialize it first).
	/// After migrate goes live on MainNet, return this implementation for next
	/// updates.
	#[private]
	#[init(ignore_state)]
	pub fn migrate() -> Self {
...
pub extern "C" fn store_blob() {
	...
	let storage_cost = ((blob_len + 32) as u128) * env::storage_byte_cost();
	assert!(env::attached_deposit() >= storage_cost, "ERR_NOT_ENOUGH_DEPOSIT:{}", storage_cost);

	env::storage_write(&sha256_hash, &input);
	let mut blob_hash = [0u8; 32];
	blob_hash.copy_from_slice(&sha256_hash);
	contract.blobs.insert(&blob_hash, &env::predecessor_account_id());
	let blob_hash_str =
		near_sdk::serde_json::to_string(&Base58CryptoHash::from(blob_hash)).unwrap().into_bytes();
...
mod tests {
	use {super::*,
	     crate::proposals::ProposalStatus,
	     near_sdk::{test_utils::{accounts, VMContextBuilder},
	                testing_env},
	     near_sdk_sim::to_yocto};

	fn create_proposal(context: &mut VMContextBuilder, contract: &mut Contract) -> u64 {
		testing_env!(context.attached_deposit(to_yocto("1")).build());
		contract.add_proposal(ProposalInput { description: "test".to_string(),
		                                      kind:
			                                      ProposalKind::Transfer { token_id:
				                                                               String::from(OLD_BASE_TOKEN),
			                                                               receiver_id:
				                                                               accounts(2).into(),
			                                                               amount:
				                                                               U128(to_yocto("100")),
	...
	fn test_basics() {
		...
		let mut contract =
			Contract::new(Config::test_config(), VersionedPolicy::Default(vec![accounts(1).into()]));
		...
		assert_eq!(contract.get_proposal(id).proposal.status, ProposalStatus::Approved);

		let id = create_proposal(&mut context, &mut contract);
		// proposal expired, finalize.
		testing_env!(context.block_timestamp(1_000_000_000 * 24 * 60 * 60 * 8).build());
		contract.act_proposal(id, Action::Finalize, None);
		assert_eq!(contract.get_proposal(id).proposal.status, ProposalStatus::Expired);

		// non council adding proposal per default policy.
		testing_env!(context.predecessor_account_id(accounts(2))
	...
	fn test_remove_proposal_allowed() {
		...
		policy.to_policy_mut().roles[1].permissions.insert("*:RemoveProposal".to_string());
