...
/// Info about factory that deployed this contract and if auto-update is
/// allowed.
...
/// By design not using contract STATE to allow for upgrade of stuck contracts
/// from factory.
pub(crate) fn internal_get_factory_info() -> FactoryInfo {
	env::storage_read(FACTORY_KEY).map(|value| {
		                              FactoryInfo::try_from_slice(&value).expect("INTERNAL_FAIL")
	                              })
	                              .unwrap_or_else(|| FactoryInfo { factory_id:
		                                                               get_default_factory_id(),
	                                                               auto_update: true, })
}

pub(crate) fn internal_set_factory_info(factory_info: &FactoryInfo) {
	env::storage_write(FACTORY_KEY, &factory_info.try_to_vec().expect("INTERNAL_FAIL"));
}
...
pub fn update() {
	...
	assert!(env::predecessor_account_id() == current_id
	...
	env::promise_batch_action_function_call(promise_id,
	                                        "migrate",
	                                        &[],
	                                        NO_DEPOSIT,
	                                        env::prepaid_gas()
...
pub(crate) fn upgrade_using_factory(code_hash: Base58CryptoHash) {
	...
	// Call `update` method from the factory which calls `update` method on this
	// account.
