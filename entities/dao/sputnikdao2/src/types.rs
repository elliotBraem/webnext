...
pub enum Action {
	...
	AddProposal,
	/// Action to remove given proposal. Used for immediate deletion in special
	/// cases.
	RemoveProposal,
...
/// In near-sdk v3, the token was represented by a String, with no other
/// restrictions. That being said, Sputnik used "" (empty String) as a
/// convention to represent the $NEAR token. In near-sdk v4, the token
/// representation was replaced by AccountId (which is in fact a wrapper
/// over a String), with the restriction that the token must be between 2 and 64
/// chars. Sputnik had to adapt since "" was not allowed anymore and we chose to
/// represent the token as a Option<AccountId> with the convention that None
/// represents the $NEAR token. This function is required to help with the
/// transition and keep the backward compatibility.
