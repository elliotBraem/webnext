...
impl Contract {
	...
	pub(crate) fn internal_add_bounty(&mut self, bounty: &Bounty) -> u64 {
		...
		self.bounties.insert(&id, &VersionedBounty::Default(bounty.clone()));
		self.last_bounty_id += 1;
		id
	}

	/// This must be called when proposal to payout bounty has been voted either
	/// successfully or not.
	pub(crate) fn internal_execute_bounty_payout(&mut self, id: u64, receiver_id: &AccountId,
	                                             success: bool)
	                                             -> PromiseOrValue<()> {
...
impl Contract {
	...
	pub fn bounty_claim(&mut self, id: u64, deadline: U64) {
		...
		assert_eq!(env::attached_deposit(), policy.bounty_bond.0, "ERR_BOUNTY_WRONG_BOND");
		...
		assert!(deadline.0 <= bounty.max_deadline.0, "ERR_BOUNTY_WRONG_DEADLINE");
		self.bounty_claims_count.insert(&id, &(claims_count + 1));
		let mut claims = self.bounty_claimers.get(&env::predecessor_account_id()).unwrap_or_default();
		claims.push(BountyClaim { bounty_id: id,
		                          start_time: U64::from(env::block_timestamp()),
		                          deadline,
		                          completed: false });
		self.bounty_claimers.insert(&env::predecessor_account_id(), &claims);
	...
	fn internal_get_claims(&mut self, id: u64, sender_id: &AccountId) -> (Vec<BountyClaim>, usize) {
		let claims = self.bounty_claimers.get(&sender_id).expect("ERR_NO_BOUNTY_CLAIMS");
		let claim_idx = self.internal_find_claim(id, &claims).expect("ERR_NO_BOUNTY_CLAIM");
		(claims, claim_idx)
	}

	/// Report that bounty is done. Creates a proposal to vote for paying out the
	/// bounty. Only creator of the claim can call `done` on bounty that is still
	/// in progress. On expired, anyone can call it to free up the claim slot.
	#[payable]
	pub fn bounty_done(&mut self, id: u64, account_id: Option<AccountId>, description: String) {
		...
		if env::block_timestamp() > claims[claim_idx].start_time.0 + claims[claim_idx].deadline.0 {
			...
			assert_eq!(sender_id, env::predecessor_account_id(), "ERR_BOUNTY_DONE_MUST_BE_SELF");
			self.add_proposal(ProposalInput { description,
			                                  kind: ProposalKind::BountyDone { bounty_id:   id,
			                                                                   receiver_id:
				                                                                   sender_id.clone(), } });
	...
	pub fn bounty_giveup(&mut self, id: u64) -> PromiseOrValue<()> {
		                ...
		                > policy.bounty_forgiveness_period.0
		{
			...
			Promise::new(env::predecessor_account_id()).transfer(policy.bounty_bond.0).into()
		};
...
mod tests {
	use {super::*,
	     crate::{proposals::{ProposalInput, ProposalKind},
	             Action, Config},
	...
	fn test_bounty_lifecycle() {
		...
		assert_eq!(contract.get_proposal(1).proposal.kind.to_policy_label(), "bounty_done");
