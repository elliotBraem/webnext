use {near_sdk::{borsh::{self, BorshDeserialize, BorshSerialize},
                ...
                serde_json::json,
                AccountId},
     near_sdk_sim::{call, init_simulator, to_yocto, DEFAULT_GAS},
     sputnikdao2::{Action, Config, ProposalInput, ProposalKind, VersionedPolicy}};
...
fn test_upgrade_using_factory() {
	...
	let hash = factory.user_account
	                  .view(factory.user_account.account_id.clone(), "get_default_code_hash", &[])
	                  .unwrap_json::<Base58CryptoHash>();

	let proposal_id = root.call(
	...
	root.call(dao_account_id.clone(),
	          "act_proposal",
	          &json!({ "id": 0, "action": Action::VoteApprove}).to_string().into_bytes(),
	          near_sdk_sim::DEFAULT_GAS,
	          0)
	    .assert_success();
}
...
fn test_upgrade_other() {
	...
	let hash = root.call(dao.user_account.account_id.clone(),
	                     "store_blob",
	                     &OTHER_WASM_BYTES,
	                     near_sdk_sim::DEFAULT_GAS,
	                     to_yocto("200"))
	...
	add_proposal(&root, &dao, ProposalInput { description: "test".to_string(),
	                                          kind:
		                                          ProposalKind::UpgradeRemote { receiver_id:
			                                                                        ref_account_id.clone(),
		                                                                        method_name:
			                                                                        "upgrade".to_string(),
		                                                                        hash }, }).assert_success();
