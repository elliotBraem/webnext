use {crate::utils::*,
     near_sdk::{env, json_types::U128, serde_json::json, AccountId},
     near_sdk_sim::{call, init_simulator, to_yocto, view},
     sputnik_staking::User,
     sputnikdao2::{default_policy, Action, BountyClaim, BountyOutput, Config, Policy, Proposal,
                   ProposalInput, ProposalKind, ProposalOutput, ProposalStatus, RoleKind,
...
fn test_large_policy() {
	...
	factory.user_account
	       .call(factory.user_account.account_id.clone(), "new", &[], near_sdk_sim::DEFAULT_GAS, 0)
	       .assert_success();

	let config = Config { name:     "testdao".to_string(),
	...
	for council_no in 0 .. NO_OF_COUNCILS {
		...
		let role = RolePermission { name:        format!("council{}", council_no),
	...
	let params = json!({ "config": config, "policy": policy }).to_string().into_bytes();

	call!(root,
	      factory.create(AccountId::new_unchecked("testdao".to_string()), Base64VecU8(params)),
	      deposit = to_yocto("10")).assert_success();

	let dao_account_id = AccountId::new_unchecked("testdao.factory".to_string());
	let dao_list = factory.user_account
...
fn test_multi_council() {
	...
	let new_policy = Policy { roles:                     vec![
            RolePermission {
                name: "all".to_string(),
...
fn test_bounty_workflow() {
	...
	call!(root, dao.act_proposal(proposal_id, Action::VoteApprove, None)).assert_success();

	let bounty_id = view!(dao.get_last_bounty_id()).unwrap_json::<u64>() - 1;
	assert_eq!(bounty_id, 0);
	assert_eq!(view!(dao.get_bounty(bounty_id)).unwrap_json::<BountyOutput>().bounty.times, 3);

	assert_eq!(to_yocto("1000"), user1.account().unwrap().amount);
	call!(user1, dao.bounty_claim(bounty_id, U64::from(0)), deposit = to_yocto("1")).assert_success();
	assert!(user1.account().unwrap().amount < to_yocto("999"));
	assert_eq!(view!(dao.get_bounty_claims(user1.account_id())).unwrap_json::<Vec<BountyClaim>>()
	                                                           .len(),
	           1);
	assert_eq!(view!(dao.get_bounty_number_of_claims(bounty_id)).unwrap_json::<u64>(), 1);
	...
	assert_eq!(view!(dao.get_bounty_claims(user1.account_id())).unwrap_json::<Vec<BountyClaim>>()
	                                                           .len(),
	           0);
	assert_eq!(view!(dao.get_bounty_number_of_claims(bounty_id)).unwrap_json::<u64>(), 0);

	assert_eq!(to_yocto("1000"), user2.account().unwrap().amount);
	call!(user2,
	      dao.bounty_claim(bounty_id, U64(env::block_timestamp() + 5_000_000_000)),
	      deposit = to_yocto("1")).assert_success();
	assert!(user2.account().unwrap().amount < to_yocto("999"));
	assert_eq!(view!(dao.get_bounty_claims(user2.account_id())).unwrap_json::<Vec<BountyClaim>>()
	                                                           .len(),
	           1);
	assert_eq!(view!(dao.get_bounty_number_of_claims(bounty_id)).unwrap_json::<u64>(), 1);
	...
	assert_eq!(view!(dao.get_proposal(proposal_id)).unwrap_json::<ProposalOutput>()
	                                               .proposal
	                                               .kind
	                                               .to_policy_label(),
	           "bounty_done");
	...
	assert_eq!(view!(dao.get_bounty_claims(user2.account_id())).unwrap_json::<Vec<BountyClaim>>()
	           ...
	           0);
	assert_eq!(view!(dao.get_bounty_number_of_claims(bounty_id)).unwrap_json::<u64>(), 0);
	assert_eq!(view!(dao.get_bounty(bounty_id)).unwrap_json::<BountyOutput>().bounty.times, 2);
}
...
fn test_create_dao_and_use_token() {
	...
	assert!(view!(dao.get_staking_contract()).unwrap_json::<String>().is_empty());
	...
	assert_eq!(policy.roles[1].kind,
	           RoleKind::Group(vec![root.account_id.clone(),
	                                user2.account_id.clone(),
	                                user3.account_id.clone()].into_iter()
	                                                       .collect()));
	add_proposal(
        &user2,
        &dao,
	...
	assert!(!view!(dao.get_staking_contract()).unwrap_json::<String>().is_empty());
	assert_eq!(view!(dao.get_proposal(2)).unwrap_json::<Proposal>().status, ProposalStatus::Approved);

	staking.user_account.view_method_call(staking.contract.ft_total_supply());
	assert_eq!(view!(staking.ft_total_supply()).unwrap_json::<U128>().0, to_yocto("0"));
	call!(user2, test_token.mint(user2.account_id.clone(), U128(to_yocto("100")))).assert_success();
	call!(user2,
	      test_token.storage_deposit(Some(staking.account_id()), None),
	      deposit = to_yocto("1")).assert_success();
	call!(user2, staking.storage_deposit(None, None), deposit = to_yocto("1"));
	call!(user2,
	      test_token.ft_transfer_call(staking.account_id(),
	                                  U128(to_yocto("10")),
	                                  None,
	                                  "".to_string()),
	      deposit = 1).assert_success();
	assert_eq!(view!(staking.ft_total_supply()).unwrap_json::<U128>().0, to_yocto("10"));
	let user2_id = user2.account_id.clone();
	assert_eq!(view!(staking.ft_balance_of(user2_id.clone())).unwrap_json::<U128>().0,
	           to_yocto("10"));
	assert_eq!(view!(test_token.ft_balance_of(user2_id.clone())).unwrap_json::<U128>().0,
	           to_yocto("90"));
	call!(user2, staking.withdraw(U128(to_yocto("5")))).assert_success();
	assert_eq!(view!(staking.ft_total_supply()).unwrap_json::<U128>().0, to_yocto("5"));
	assert_eq!(view!(test_token.ft_balance_of(user2_id.clone())).unwrap_json::<U128>().0,
	           to_yocto("95"));
	call!(user2, staking.delegate(user2_id.clone(), U128(to_yocto("5")))).assert_success();
	call!(user2, staking.undelegate(user2_id.clone(), U128(to_yocto("1")))).assert_success();
	// should fail right after undelegation as need to wait for voting period before
	// can delegate again.
	should_fail(call!(user2, staking.delegate(user2_id.clone(), U128(to_yocto("1")))));
	let user = view!(staking.get_user(user2_id.clone())).unwrap_json::<User>();
	assert_eq!(user.delegated_amounts, vec![(user2_id.clone(), U128(to_yocto("4")))]);
	assert_eq!(view!(dao.delegation_total_supply()).unwrap_json::<U128>().0, to_yocto("4"));
	assert_eq!(view!(dao.delegation_balance_of(user2_id.clone())).unwrap_json::<U128>().0,
	           to_yocto("4"));
}
...
fn test_failures() {
	let (root, dao) = setup_dao();
	should_fail(add_transfer_proposal(&root,
	                                  &dao,
	                                  base_token(),
	                                  user(1),
	                                  1_000_000,
...
fn test_payment_failures() {
	...
	call!(user1,
	      ...
	      deposit = to_yocto("125")).assert_success();
	...
	call!(whale, test_token.mint(whale.account_id.clone(), U128(6_000_000_000))).assert_success();
	call!(whale,
	      test_token.ft_transfer(dao.account_id(),
	                             U128::from(1000),
