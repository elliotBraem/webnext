...
use {near_sdk::{env, json_types::U128, AccountId, Balance},
     near_sdk_sim::{call, deploy, init_simulator, to_yocto, transaction::ExecutionStatus,
                    ContractAccount, ExecutionResult, UserAccount},
     sputnik_staking::ContractContract as StakingContract,
     sputnikdao2::{Action, Bounty, Config, ContractContract as DAOContract, OldAccountId,
                   ProposalInput, ProposalKind, VersionedPolicy, OLD_BASE_TOKEN},
     sputnikdao_factory2::SputnikDAOFactoryContract as FactoryContract,
     test_token::ContractContract as TestTokenContract};
...
pub fn add_proposal(root: &UserAccount, dao: &Contract, proposal: ProposalInput)
                    -> ExecutionResult {
	call!(root, dao.add_proposal(proposal), deposit = to_yocto("1"))
}

pub fn add_member_proposal(root: &UserAccount, dao: &Contract, member_id: AccountId)
                           -> ExecutionResult {
	add_proposal(root, dao, ProposalInput { description: "test".to_string(),
	                                        kind:
		                                        ProposalKind::AddMemberToRole { member_id,
		                                                                        role:
			                                                                        "council".to_string() }, })
}

pub fn add_transfer_proposal(root: &UserAccount, dao: &Contract, token_id: Option<AccountId>,
                             receiver_id: AccountId, amount: Balance, msg: Option<String>)
                             -> ExecutionResult {
	add_proposal(root,
	             dao,
	             ProposalInput { description: "test".to_string(),
	                             kind:
		                             ProposalKind::Transfer { token_id:
			                                                      convert_new_to_old_token(token_id),
		                                                      receiver_id,
		                                                      amount: U128(amount),
		                                                      msg }, })
}
...
pub fn add_bounty_proposal(root: &UserAccount, dao: &Contract) -> ExecutionResult {
	add_proposal(
				...
				OLD_BASE_TOKEN,
			                                                                ...
			                                                                amount:       U128(to_yocto(
				"10",
			)),
			                                                                times:        3,
			                                                                max_deadline: U64(
				env::block_timestamp() + 10_000_000_000,
			), }, }, },
	)
}
...
pub fn vote(users: Vec<&UserAccount>, dao: &Contract, proposal_id: u64) {
	for user in users.into_iter() {
		call!(user, dao.act_proposal(proposal_id, Action::VoteApprove, None)).assert_success();
	}
}
